...
def parse_args():
    parser = argparse.ArgumentParser(description="Train HeteroGNN with contrastive learning")
    parser.add_argument('--projectName', type=str, required=True, help='Project name (subfolder under data/graph/)')
    parser.add_argument('--fold', type=str, default='0', help='Fold number, e.g., 0,1,2,3,4')
...
if __name__ == "__main__":
    ...
    # 加载数据
    fold_dir = 'data/graph/' + args.projectName+ '/fold_' + args.fold
    ...
    graph_jaccard['lncRNA', 'jaccard_related', 'lncRNA'].edge_index = torch.empty(2, 0, dtype=torch.long)
    graph_jaccard['protein', 'jaccard_related', 'protein'].edge_index = torch.empty(2, 0, dtype=torch.long)
    # 预加载图数据到GPU
    x_dict_jaccard = {
        'lncRNA': graph_jaccard['lncRNA'].x.to(device),
        'protein': graph_jaccard['protein'].x.to(device)
